...
const ThirdPartyCostForm = (props?:any) => {
  ...
  return (
    <React.Fragment>
           {props.getSystemAccess && props.getSystemAccess.systemAceess && props.getSystemAccess.systemAceess.length?
           <Box sx={{ marginTop: 5, padding: 5 }}>
           <Typography sx={{ flex: 1, marginBottom: 3 }} variant='h4' component='div'>
               Third Party Cost Setup
           </Typography>
           <Box sx={{ border: "1px solid #ccc", padding: 5 }}>
               <Formik
                   ...
                   onSubmit={(values, { setSubmitting, resetForm }) => {
                       const postData: any = {
                           third_party_cost_info: props.setBizCaseSetupData,
                           third_party_cost_data: values.systemAceess
                       }
                                   ...
                                   <Grid item xs={12} style={{ marginTop: 0 }}>
                                       <FieldArray
                                           ...
                                           render={({ insert, remove, push }) => (
                                               <Box>
                                                   {
                                                      facilityValue && facilityValue.systemAceess ?
                                                           facilityValue.systemAceess.map((facilityProperty: any, index: any) => (
                                                               <Box className='flexcolumbox' sx={{marginBottom:5}} key={index}>
                                                              {/* <Grid key={index} container direction="row" alignItems="center" */}
                                                                   ...
                                                                   <Grid item xs={12} md={2} sx={{marginRight:2}}>
                                                                       <DatePicker
                                                                           ...
                                                                           onChange={(data) => {
                                                                               ...
                                                                               facilityProperty.setFieldValue(`systemAceess.${index}.target_date`, systemAccessTargetDate.format("YYYY-MM-DD"));
                                                                           }}
                                                                           disabled={true}
                                                                   ...
                                                                   <Grid item xs={12} md={2} sx={{marginRight:2}}>
                                                                       <FormControl variant="outlined" fullWidth margin='dense'>
                                                                           <TextField name={`systemAceess.${index}.business_year-label`}
                                                                               value={facilityProperty?.business_year}
                                                                               label='Business Year' variant='outlined' onChange={handleChange}
                                                                               disabled />
                                                                       </FormControl>
                                                                   </Grid>
                                                                   <Grid item xs={12} md={3} sx={{marginRight:2}}>
...
export default ThirdPartyCostForm;
